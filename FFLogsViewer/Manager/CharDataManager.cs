...
public class CharDataManager
{
    ...
    public void UpdatePartyMembers(bool forceLocalPlayerParty = true)
    {
        ...
        {
            var index = currPartyMembers.FindIndex(member => $"{member.FirstName}" == localPLayer?.CharacterName
        ...
        {
            var member = this.PartyMembers.FirstOrDefault(x => x.FirstName == partyMember.FirstName && x.WorldName == partyMember.World);
            if (member == null)
            {
                // add new member
                this.PartyMembers.Add(new CharData(partyMember.FirstName, partyMember.World, partyMember.JobId));
            }
            else
            {
        ...
        this.PartyMembers.RemoveAll(x => !currPartyMembers.Any(y => y.FirstName == x.FirstName && y.World == x.WorldName));
